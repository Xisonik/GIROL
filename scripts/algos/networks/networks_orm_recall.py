...
class MLP(nn.Module):
    def __init__(self, input_dim, output_dim, activation=nn.ReLU, output_activation=None):
        ...
        layers = [
            nn.Linear(input_dim, 256), activation(),
...
class TaskEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dims=(32,), output_dim=task_emb_dim):
        super().__init__()
...
class DualPathMLP(nn.Module):
    """
    Двухпоточный MLP:
    - Основной поток: обрабатывает img + graph
    - Дополнительный поток: task_emb (копируется и добавляется к каждому слою основного)
    """
    def __init__(self, img_graph_dim, task_emb_dim, output_dim, hidden_dims=[256, 256], activation=nn.ReLU, output_activation=nn.Tanh):
        super().__init__()
        
        # Основной MLP для img+graph
        self.main_layers = nn.ModuleList()
        prev_dim = img_graph_dim
        
        for hidden_dim in hidden_dims:
            self.main_layers.append(nn.Linear(prev_dim + task_emb_dim, hidden_dim))
            self.main_layers.append(activation())
            prev_dim = hidden_dim
        
        # Выходной слой
        self.output_layer = nn.Linear(prev_dim + task_emb_dim, output_dim)
        self.output_activation = output_activation() if output_activation else nn.Identity()
        
    def forward(self, img_graph, task_emb):
        """
        Args:
            img_graph: [batch, img_graph_dim]
            task_emb: [batch, task_emb_dim]
        """
        x = img_graph
        
        # Проходим через каждый слой основного MLP
        for i in range(0, len(self.main_layers), 2):
            linear = self.main_layers[i]
            activation = self.main_layers[i+1]
            
            # Конкатенируем task_emb с текущим представлением
            x = torch.cat([x, task_emb], dim=-1)
            x = linear(x)
            x = activation(x)
        
        # Финальный слой с task_emb
        x = torch.cat([x, task_emb], dim=-1)
...
class StochasticActor(GaussianMixin, Model):
    def __init__(self, observation_space, action_space, device,
                 ...
                 clip_actions=False, clip_log_std=True, min_log_std=-5, max_log_std=2, use_graph=True):
        ...
        self.task_emb_dim = task_emb_dim  # нужно определить где-то, например 64 или 128
        img_graph_dim = img_dim + graph_dim
        
        # Используем двухпоточный MLP
        self.net = DualPathMLP(
            img_graph_dim=img_graph_dim,
            task_emb_dim=self.task_emb_dim,
            output_dim=self.num_actions,
            hidden_dims=[256, 256],  # можно настроить
            activation=nn.ReLU,
            output_activation=nn.Tanh
        )
        
        self.task_encoder = TaskEncoder(task_in, output_dim=self.task_emb_dim)
    ...
    def compute(self, inputs, role):
        ...
        # Объединяем img и graph
        if self.use_graph:
            img_graph = torch.cat([img, graph_emb], dim=-1)
        else:
            img_graph = img
        
        # Прогоняем через двухпоточный MLP
        return self.net(img_graph, task_emb), self.log_std_parameter, {}


class Critic(DeterministicMixin, Model):
    def __init__(self, observation_space, action_space, device,
                 graph_encoder, orient_module, clip_actions=False, use_graph=True):
        Model.__init__(self, observation_space, action_space, device)
        ...
        # Для критика нужно добавить actions, поэтому отдельный класс
        self.net = CriticDualPathMLP(
            img_graph_dim=img_graph_dim,
            task_emb_dim=self.task_emb_dim,
            actions_dim=self.num_actions,
            hidden_dims=[256, 256],
            activation=nn.ReLU
        )
    ...
    def compute(self, inputs, role):
        ...
        return self.net(img_graph, task_emb, actions), {}


class CriticDualPathMLP(nn.Module):
    """Версия для критика с actions"""
    def __init__(self, img_graph_dim, task_emb_dim, actions_dim, output_dim=1, 
                 hidden_dims=[256, 256], activation=nn.ReLU):
        super().__init__()
        
        self.main_layers = nn.ModuleList()
        prev_dim = img_graph_dim + actions_dim
    ...
    def forward(self, img_graph, task_emb, actions):
        x = torch.cat([img_graph, actions], dim=-1)
