...
class StochasticActor(GaussianMixin, Model):
    def __init__(self, observation_space, action_space, device,
        ...
        graph_dim = int(observation_space["graph"].shape[0])
        mlp_in = img_dim + goal_dim + graph_dim + 1 # img + graph_emb + pred_angle
    ...
    def compute(self, inputs, role):
        ...
        x = torch.cat([img, goal, graph_flat, gt_orientation], dim=-1)
...
class Critic(DeterministicMixin, Model):
    def __init__(self, observation_space, action_space, device,
        ...
        goal_dim = int(observation_space["goal"].shape[0])
        graph_dim = int(observation_space["graph"].shape[0])
        mlp_in = img_dim + goal_dim + graph_dim + self.num_actions + 1
    ...
    def compute(self, inputs, role):
        ...
        x = torch.cat([img, goal, graph_flat, gt_orientation, actions], dim=-1)
        return self.net(x), {}
