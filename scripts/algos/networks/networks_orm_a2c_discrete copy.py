...
# class StochasticActor(MultiCategoricalMixin, Model):
#     def __init__(
#         self,
#         observation_space,
#         action_space,
...
#         mlp_in = img_dim + graph_dim + task_out

#         # Для MultiDiscrete([2, 5]) self.num_actions должен быть 2 + 5 = 7 logits
#         # Первые 2 logits — linear action, следующие 5 — angular action
#         self.net = MLP(
#             input_dim=mlp_in,
...
#         # MultiCategoricalMixin ждёт logits
#         return self.net(x), {}

class StochasticActor(CategoricalMixin, Model):
    def __init__(
        self,
        observation_space,
        action_space,
        ...
        # Discrete(3):
        #   logit 0 -> turn_left_30
        #   logit 1 -> turn_right_30
        #   logit 2 -> move_forward_25cm
        self.net = MLP(
            input_dim=mlp_in,
            output_dim=self.num_actions,
            output_activation=None,
...
class Critic(DeterministicMixin, Model):
    def __init__(self, observation_space, action_space, device,
        ...
        mlp_in = img_dim + graph_dim + self.num_actions + task_out
        self.net = MLP(
            input_dim=mlp_in,
            output_dim=1,
        )
