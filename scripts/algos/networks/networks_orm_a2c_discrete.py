"""
SAC/A2C + отдельно обучаемые GraphEncoder и OrientationModule
---------------------------------------------------------
Архитектура:
  - GraphEncoder: CLIP text lookup + GATv2 → graph_emb (128)
  - OrientationModule: img + graph_emb → orientation angle
  - Actor/Critic/Value: используют graph_emb и orientation через no_grad
...
from skrl.models.torch import DeterministicMixin, CategoricalMixin, MultiCategoricalMixin, Model
...
# =====================================================================
class MLP(nn.Module):
    def __init__(self, input_dim, output_dim, activation=nn.ReLU, output_activation=None):
        super().__init__()

        layers = [
            nn.Linear(input_dim, 32), activation(),
...
class TaskEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dims=(32,), output_dim=64):
        super().__init__()

        layers = []
        prev = input_dim
        hidden_dims = (int(output_dim/2),)
...
class StochasticActor(Model):
    """Auto-detects action space type and wraps appropriate Mixin."""
    
    @staticmethod
    def create(observation_space, action_space, device, **kwargs):
        """Factory method that returns the right Actor type."""
        if isinstance(action_space, gym.spaces.Discrete):
            return _DiscreteActor(observation_space, action_space, device, **kwargs)
        elif isinstance(action_space, gym.spaces.MultiDiscrete):
            return _MultiDiscreteActor(observation_space, action_space, device, **kwargs)
        elif isinstance(action_space, gym.spaces.Box):
            return _ContinuousActor(observation_space, action_space, device, **kwargs)  # если нужен
        else:
            raise ValueError(f"Unsupported action space: {type(action_space)}")


class _DiscreteActor(CategoricalMixin, Model):
    """Actor for Discrete action space (e.g., Discrete(10))."""
    def __init__(self, observation_space, action_space, device,
                 graph_encoder, orient_module, unnormalized_log_prob=True,
                 use_graph=True, task_out=64):
        Model.__init__(self, observation_space, action_space, device)
        CategoricalMixin.__init__(self, unnormalized_log_prob=unnormalized_log_prob)
        self._init_common(observation_space, action_space, device,
                         graph_encoder, orient_module, use_graph, task_out)
        
        mlp_in = self._get_mlp_input_dim()
        self.net = MLP(input_dim=mlp_in, output_dim=self.num_actions, output_activation=None)
    
    def _init_common(self, obs_space, act_space, device, graph_encoder, orient_module, use_graph, task_out):
        self.__dict__["graph_encoder"] = graph_encoder
        self.__dict__["orient_module"] = orient_module
        self.use_graph = use_graph
        self.img_dim = int(obs_space["img"].shape[0])
        self.goal_dim = int(obs_space["goal"].shape[0])
        self.graph_dim = 128 if use_graph else 0
        self.task_in = self.goal_dim + 1
        self.use_encoder = task_out > 0
        if not self.use_encoder:
            self.task_out = self.task_in
        else:
            self.task_out = task_out
            self.task_encoder = TaskEncoder(input_dim=self.task_in, output_dim=self.task_out)
    
    def _get_mlp_input_dim(self):
        return self.task_out # self.img_dim + self.graph_dim + 
    
    def compute(self, inputs, role):
        states = unflatten_tensorized_space(self.observation_space, inputs["states"])
        task_emb, graph_emb = self._process_common(states)
        x = torch.cat([task_emb], dim=-1)
        # if self.use_graph:
        #     x = torch.cat([states["img"], graph_emb, task_emb], dim=-1)
        # else:
        #     x = torch.cat([states["img"], task_emb], dim=-1)
        
        return self.net(x), {}
    
    def _process_common(self, states):
        with torch.no_grad():
            graph_emb = self.graph_encoder(states["graph"])
            pred_angle, _, _ = self.orient_module(states["img"], graph_emb)
            # For data collection
            collect_orientation_data(states["orientation"], pred_angle)
            print_orientation_accuracy()
        
        task = torch.cat([states["goal"], states["orientation"]], dim=-1)
        if self.use_encoder:
            task_emb = self.task_encoder(task)
        else:
            task_emb = task
        return task_emb, graph_emb


class _MultiDiscreteActor(MultiCategoricalMixin, Model):
    """Actor for MultiDiscrete action space (e.g., MultiDiscrete([2, 5]))."""
    def __init__(self, observation_space, action_space, device,
                 graph_encoder, orient_module, unnormalized_log_prob=True,
                 reduction="sum", use_graph=True, task_out=64):
        Model.__init__(self, observation_space, action_space, device)
        ...
        mlp_in = self._get_mlp_input_dim()
        # MultiDiscrete: self.num_actions = sum of nvec (e.g., 2+5=7)
        self.net = MLP(input_dim=mlp_in, output_dim=self.num_actions, output_activation=None)
    
    # Reuse _init_common, _get_mlp_input_dim, _process_common from _DiscreteActor
    _init_common = _DiscreteActor._init_common
    _get_mlp_input_dim = _DiscreteActor._get_mlp_input_dim
    ...
    _process_common = _DiscreteActor._process_common


class Critic(DeterministicMixin, Model):
    def __init__(self, observation_space, action_space, device,
                 graph_encoder, orient_module, clip_actions=False, use_graph=True, task_out=64):
        Model.__init__(self, observation_space, action_space, device)
        ...
        graph_dim = 128 if use_graph else 0
        task_in = goal_dim + 1  # goal + orientation
        self.use_encoder = task_out > 0
        if not self.use_encoder:
            task_out = task_in
        else:
            self.task_encoder = TaskEncoder(input_dim = task_in, output_dim = task_out)

        mlp_in = self.num_actions + task_out #img_dim + graph_dim + 
        self.net = MLP(
            input_dim=mlp_in,
            output_dim=1,
        )
    ...
    def compute(self, inputs, role):
        ...
        task = torch.cat([goal, gt_orientation], dim=-1)
        if self.use_encoder:
            task_emb = self.task_encoder(task)
        else:
            task_emb = task
        x = torch.cat([task_emb, actions], dim=-1)
        # if self.use_graph:
        #     x = torch.cat([img, graph_emb, task_emb, actions], dim=-1)
        # else:
        #     x = torch.cat([img, task_emb, actions], dim=-1)
        return self.net(x), {}


class ValueFunction(DeterministicMixin, Model):
    """A2C value network: V(s). Unlike SAC Critic/Q, it does not consume actions."""
    def __init__(self, observation_space, action_space, device,
                 graph_encoder, orient_module, clip_actions=False, use_graph=True, task_out=64):
        Model.__init__(self, observation_space, action_space, device)
        DeterministicMixin.__init__(self, clip_actions)

        # Не регистрируем — основной RL optimizer их не увидит
        self.__dict__["graph_encoder"] = graph_encoder
        self.__dict__["orient_module"] = orient_module
        ...
        mlp_in = task_out #img_dim + graph_dim + 
        self.net = MLP(
            input_dim=mlp_in,
            output_dim=1,
        )
    ...
    def compute(self, inputs, role):
        ...
        x = torch.cat([task_emb], dim=-1)
        # if self.use_graph:
        #     x = torch.cat([img, graph_emb, task_emb], dim=-1)
        # else:
        #     x = torch.cat([img, task_emb], dim=-1)
        return self.net(x), {}
